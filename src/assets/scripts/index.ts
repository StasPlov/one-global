...
addEventListener("DOMContentLoaded", () => {
	...
	observeVisibility('stopAnimTrigger')

	function observeVisibility(elementId: string) {
		...
		var observer = new IntersectionObserver(function (entries) {
			...
			if (!entry.isIntersecting && entry.intersectionRect.top < 0) {
				// Если элемент не видимый (пропал из зоны видимости)
				const el = document.getElementById('stopAnimTriggerClose');
				if (!el) {
					return; // Если элемент не найден, просто выходим из функции
				}
				el.classList.add('hidden'); // Изменяем стиль элемента
				observer.disconnect(); // Отключаем наблюдение
			}
		});

		observer.observe(element); // Запускаем наблюдение за элементом
	}

	// Пример использования
	observeVisibility('myElement'); // Замените 'myElement' на идентификатор вашего элемента
});