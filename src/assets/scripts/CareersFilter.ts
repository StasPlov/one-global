...
export default class CareersFilter {
	...
	init() {
		...
		if (this.careersFilter !== null && this.careersList !== null) {
			this.careersFilter.addEventListener('submit', async (event) => {
				event.preventDefault();
				if (this.isSendingForm) {
					return;
				}

				/* const loader = ((event.target as HTMLElement).querySelector('button[type="button"]') as HTMLElement).querySelector('#loader') as HTMLElement;
				if(loader !== null) {
					loader.classList.add('hidden');
				} */

				const url = this.careersFilter?.getAttribute('data-endpoint');

				if (url) {
					this.isSendingForm = true;
					var formData = new FormData(this.careersFilter as HTMLFormElement);
					...
					// add data to page 
					result.forEach(item => {
						let htmlItem = `<div class="flex border border-solid border-[#1c212633] max-md:flex-[100%] max-md:flex-wrap max-md:border-x-0 max-md:border-t-0">
